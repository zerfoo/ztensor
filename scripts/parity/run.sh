...
grep -q -- '--- PASS: TestAttentionTraining_WolfPattern_GPU' /tmp/out.txt || { echo FATAL: Wolf-pattern attention stress not PASS; exit 7; }
...
echo "reports:"
ls -l "$REPORT_DIR"
echo "VALIDATION_OK: CPU-vs-GPU parity (both schedules) + GPU red-proof + Wolf-pattern training loop + attention stress passed on GB10"
